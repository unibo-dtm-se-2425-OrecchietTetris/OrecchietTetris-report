...
- NF1: the game must run identically on Windows, macOS, and Linux
- NF2: the game must be usable without any network connection or external backend service
- NF3: the game's controls and menus must be simple enough to be understood without instructions (pick-up-and-play)
- NF4: saved leaderboard data must persist across application restarts and must not be lost on a normal game-over or quit
- NF5: the game must be installable and launchable through a minimal number of steps (ideally one command)
- NF6: the visual and audio theming must consistently reflect Apulian culture across blocks, music, and menus
...
- NF1-NF6: if achieved, this will be verified through manual playtesting and, where applicable, automated tests measuring cross-platform runs
